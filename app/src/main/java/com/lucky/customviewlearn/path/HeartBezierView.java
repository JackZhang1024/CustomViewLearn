...
public class HeartBezierView extends View {
    ...
    private float mDifference = mCircleRadius*C; // 圆的控制点俞数据点的插值

    private float[] mData = new float[8];  //顺时针纪录绘制圆形的四个数据点
    private float[] mCtrl = new float[16]; //顺时针记录绘制圆形的八个控制点

    private float mDuration = 1000; //变化时长
    private float mCurrent = 0; //当前已进行时长
    private float mCount = 100; // 将时长总共划分多少份
    private float mPiece = mDuration/mCount; //每份的时长
